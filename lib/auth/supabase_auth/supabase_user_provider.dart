...
class NoteAppSupabaseUser extends BaseAuthUser {
  NoteAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> noteAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NoteAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
